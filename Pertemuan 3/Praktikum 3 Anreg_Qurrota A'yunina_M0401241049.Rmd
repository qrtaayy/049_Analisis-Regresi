...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\USER\\Documents\\Mata Kuliah Semester 4\\Analisis Regresi\\Data Pengangguran_Qurrota A'yunina_M0401241049.xlsx", sheet="Data")
dt
```
...
summary(model)
```
Berdasarkan perhitungan, didapat dugaan persamaan regresi linear sebagai berikut. Dugaan persamaan tersebut dapat pula disebut sebagai nilai harapan dari peubah respons Y (Y=Tingkat Pengangguran Terbuka (TPT))

$$E[\hat{Y}] = \hat{Y} = -1.7326 + 0.6557 X$$
...
```
Dari scatter plot terlihat bahwa terdapat kecenderungan hubungan negatif antara variabel X dan Y, yang ditunjukkan oleh garis regresi yang menurun. Namun, penyebaran titik data relatif jauh dari garis regresi sehingga hubungan yang terjadi tidak terlalu kuat. Beberapa amatan tampak menyimpang dari garis dugaan, yang menunjukkan adanya galat yang cukup besar pada beberapa titik. Selain itu, garis regresi tidak jauh berbeda dari garis rataan respon, sehingga keragaman yang mampu dijelaskan oleh model regresi relatif kecil dibandingkan keragaman total data. Keragaman dari galat tiap amatan ini dapat diurai berdasarkan garis dugaan persamaan ($\hat{Y}$) dan garis rataan nilai resposnya ($\bar{Y}$). Penguraian keragaman ini dapat dituliskan sebagai Jumlah Kuadrat Regresi (JKR), Jumlah Kuadrat Galat (JKG), dan Jumlah Kuadrat Total (JKT) dengan perhitungan sebagai berikut.
...
$$JKT = JKR + JKG$$
Nilai penguraian keragaman ini dapat diamati pula menggunakan fungsi anova pada bagian **Sum Sq (Jumlah Kuadrat)** seperti yang ditampilkan di bawah. Pada output sintaks di bawah, JKR model bernilai 22.523 dan JKG model bernilai 48.071.

```{r}
(anova.model <- anova(model))
```
Dari tabel ANOVA dan penguraian keragaman, dapat diduga pula ragam galat/error dari model yang kita peroleh. Secara umum, error dari model dapat diduga oleh nilai Kuadrat Tengah Galat (KTG) sehingga berdasarkan output sintaks di atas, diketahui bahwa ragam galat dari model bernilai 1.3353. Hubungan antara keduanya dapat dituliskan sebagai berikut.
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{1.335311} \approx 1.155556$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar -1.7326 dan 0.6557. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [-5.417047 ,  1.951905] dan untuk parameter $β_1$ adalah [ 0.2781542 ,  1.033147 ]
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=10 adalah 4.823935. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang 4.405124 hingga 5.242745.
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=10 adalah 4.823935. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang 2.44323 hingga 7.20464.